...
//#endregion
//#region node_modules/plyr/dist/plyr.js
var require_plyr = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	typeof navigator === "object" && (function(global, factory) {
		typeof exports === "object" && typeof module !== "undefined" ? module.exports = factory() : typeof define === "function" && define.amd ? define("Plyr", factory) : (global = typeof globalThis !== "undefined" ? globalThis : global || self, global.Plyr = factory());
	})(exports, (function() {
		"use strict";
		...
		var RangeTouch = function() {
			...
			return _createClass(e, [
				...
				value: function(t) {
					...
					if (is$1.string(t) && i.watch) new MutationObserver(function(n) {
		...
		function extend(target = {}, ...sources) {
			...
			if (!is.object(source)) return target;
		...
		function createElement(type, attributes, text) {
			...
			if (is.object(attributes)) setAttributes(element, attributes);
			if (is.string(text)) element.textContent = text;
			return element;
		}
		...
		function getAttributesFromSelector(sel, existingAttributes) {
			...
			sel.split(",").forEach((s) => {
				...
				switch (selector.charAt(0)) {
					case ".":
						if (is.string(existing.class)) attributes.class = `${existing.class} ${className}`;
		...
		function toggleHidden(element, hidden) {
			if (!is.element(element)) return;
			let hide = hidden;
			if (!is.boolean(hide)) hide = !element.hidden;
			element.hidden = hide;
		}
		function toggleClass(element, className, force) {
			if (is.nodeList(element)) return Array.from(element).map((e) => toggleClass(e, className, force));
		...
		const support = {
			...
			pip: document.pictureInPictureEnabled && !createElement("video").disablePictureInPicture,
			airplay: is.function(window.WebKitPlaybackTargetAvailabilityEvent),
			playsinline: "playsInline" in document.createElement("video"),
			mime(input) {
				if (is.empty(input)) return false;
		...
		function toggleListener(element, event, callback, toggle = false, passive = true, capture = false) {
			if (!element || !("addEventListener" in element) || is.empty(event) || !is.function(callback)) return;
		...
		function triggerEvent(element, type = "", bubbles = false, detail = {}) {
			if (!is.element(element) || is.empty(type)) return;
		...
		*/
		function silencePromise(value) {
			if (is.promise(value)) value.then(null, () => {});
		}
		...
		function closest(array, value) {
			if (!is.array(array) || !array.length) return null;
		...
		function validateAspectRatio(input) {
			if (!is.array(input) && (!is.string(input) || !input.includes(":"))) return false;
			return (is.array(input) ? input : input.split(":")).map(Number).every(is.number);
		}
		function reduceAspectRatio(ratio) {
			if (!is.array(ratio) || !ratio.every(is.number)) return null;
		...
		const html5 = {
			...
			setup() {
				...
				if (!is.empty(this.config.ratio)) setAspectRatio.call(player);
				Object.defineProperty(player.media, "quality", {
					...
					set(input) {
						...
						if (player.config.quality.forced && is.function(player.config.quality.onChange)) player.config.quality.onChange(input);
						else {
		...
		const i18n = { get(key = "", config = {}) {
			if (is.empty(key) || is.empty(config)) return "";
		...
		function formatTime(time = 0, displayHours = false, inverted = false) {
			if (!is.number(time)) return formatTime(void 0, displayHours, inverted);
		...
		const controls = {
			...
			findElements() {
				try {
					...
					if (is.element(this.elements.progress)) this.elements.display.seekTooltip = this.elements.progress.querySelector(`.${this.config.classNames.tooltip}`);
					return true;
				} catch (error) {
			...
			createButton(buttonType, attr) {
				...
				switch (buttonType) {
					...
					case "play-large":
						...
						break;
					default:
						if (is.empty(props.label)) props.label = type;
				...
				if (type === "play") {
					if (!is.array(this.elements.buttons[type])) this.elements.buttons[type] = [];
					...
					if (!isRadioButton && [" ", "ArrowRight"].includes(event.key)) controls.showMenuPanel.call(this, type, true);
						...
						if (event.key !== " ") {
							if (event.key === "ArrowDown" || isRadioButton && event.key === "ArrowRight") {
								...
								if (!is.element(target)) target = menuItem.parentNode.firstElementChild;
							} else {
			...
			createMenuItem({ value, list, type, title, badge = null, checked = false }) {
				...
				if (is.element(badge)) flex.appendChild(badge);
				...
				this.listeners.bind(menuItem, "click keyup", (event) => {
					if (is.keyboardEvent(event) && event.key !== " ") return;
					event.preventDefault();
					event.stopPropagation();
			...
			updateTimeDisplay(target = null, time = 0, inverted = false) {
				if (!is.element(target) || !is.number(time)) return;
			...
			updateVolume() {
				...
				if (is.element(this.elements.buttons.mute)) this.elements.buttons.mute.pressed = this.muted || this.volume === 0;
			},
			...
			updateProgress(event) {
				...
				const setProgress = (target, input) => {
					...
					if (is.element(progress)) {
						...
						if (is.element(label)) label.childNodes[0].nodeValue = val;
					}
				};
			...
			updateRangeFill(target) {
				...
				if (!is.element(range) || range.getAttribute("type") !== "range") return;
			...
			updateSeekTooltip(event) {
				...
				if (!this.config.tooltips.seek || !is.element(this.elements.inputs.seek) || !is.element(this.elements.display.seekTooltip) || this.duration === 0) return;
				...
				if (is.event(event) && ["mouseenter", "mouseleave"].includes(event.type)) toggle(event.type === "mouseenter");
			},
			timeUpdate(event) {
			...
			updateSetting(setting, container, input) {
				...
				if (setting === "captions") value = this.currentTrack;
					...
					if (is.empty(value)) value = this.config[setting].default;
				...
				if (is.element(target)) target.checked = true;
			},
			...
			setQualityMenu(options) {
				...
				if (is.array(options)) this.options.quality = dedupe(options).filter((quality) => this.config.quality.options.includes(quality));
			...
			focusFirstMenuItem(pane, focusVisible = false) {
				...
				let target = pane;
				if (!is.element(target)) target = Object.values(this.elements.settings.panels).find((p) => !p.hidden);
			...
			toggleMenu(input) {
				...
				if (!is.element(popup) || !is.element(button)) return;
				const { hidden } = popup;
				let show = hidden;
				if (is.boolean(input)) show = input;
				else if (is.keyboardEvent(input) && input.key === "Escape") show = false;
				...
				if (show && is.keyboardEvent(input)) controls.focusFirstMenuItem.call(this, null, true);
				else if (!show && !hidden) setFocus.call(this, button, is.keyboardEvent(input));
			},
			getMenuSize(tab) {
			...
			create(data) {
				...
				if (is.array(this.config.controls) && this.config.controls.includes("play-large")) this.elements.container.appendChild(createButton.call(this, "play-large"));
				...
				dedupe(is.array(this.config.controls) ? this.config.controls : []).forEach((control) => {
					...
					if (control === "download") {
						...
						if (!is.url(download) && this.isEmbed) extend(attributes, {
			...
			inject() {
				...
				let update = true;
				if (is.function(this.config.controls)) this.config.controls = this.config.controls.call(this, props);
				if (!this.config.controls) this.config.controls = [];
				if (is.element(this.config.controls) || is.string(this.config.controls)) container = this.config.controls;
				else {
				...
				if (update) {
					if (is.string(this.config.controls)) container = replace(container);
				}
				let target;
				if (is.string(this.config.selectors.controls.container)) target = document.querySelector(this.config.selectors.controls.container);
				...
				if (!is.empty(this.elements.buttons)) {
					...
					Object.values(this.elements.buttons).filter(Boolean).forEach((button) => {
						if (is.array(button) || is.nodeList(button)) Array.from(button).filter(Boolean).forEach(addProperty);
		...
		*/
		...
		function buildUrlParams(input) {
			...
			if (is.object(input)) Object.entries(input).forEach(([key, value]) => {
		...
		const captions = {
			setup() {
				...
				if (!this.isVideo || this.isYouTube || this.isHTML5 && !support.textTracks) {
					if (is.array(this.config.controls) && this.config.controls.includes("settings") && this.config.settings.includes("captions")) controls.setCaptionsMenu.call(this);
					return;
				}
				...
				if (!is.boolean(active)) ({active} = this.config.captions);
			...
			getLabel(track) {
				...
				if (!is.track(currentTrack) && support.textTracks && this.captions.toggled) currentTrack = captions.getCurrentTrack.call(this);
				if (is.track(currentTrack)) {
					if (!is.empty(currentTrack.label)) return currentTrack.label;
		...
		*/
		...
		class Fullscreen {
			constructor(player) {
				_defineProperty$1(this, "onChange", () => {
					...
					if (is.element(button)) button.pressed = this.active;
				...
				_defineProperty$1(this, "enter", () => {
					...
					else if (!is.empty(this.prefix)) this.target[`${this.prefix}Request${this.property}`]();
				});
		...
		const ui = {
			...
			setTitle() {
				...
				if (is.string(this.config.title) && !is.empty(this.config.title)) label += `, ${this.config.title}`;
			...
			checkPlaying(event) {
				...
				if (is.event(event) && event.type === "timeupdate") return;
			...
			migrateStyles() {
				...
				if (is.empty(this.media.style)) this.media.removeAttribute("style");
			}
		};
		class Listeners {
			constructor(_player) {
				...
				_defineProperty$1(this, "proxy", (event, defaultHandler, customHandlerKey) => {
					...
					if (returned !== false && is.function(defaultHandler)) defaultHandler.call(player, event);
				});
				...
				_defineProperty$1(this, "controls", () => {
					...
					this.bind(elements.inputs.seek, "mousedown mouseup keydown keyup touchstart touchend", (event) => {
						...
						if (is.keyboardEvent(event) && !["ArrowLeft", "ArrowRight"].includes(event.key)) return;
						player.lastSeekTime = Date.now();
					...
					this.bind(elements.inputs.seek, inputEvent, (event) => {
						...
						if (is.empty(seekTo)) seekTo = seek.value;
					...
					if (browser.isWebKit) Array.from(getElements.call(player, "input[type=\"range\"]")).forEach((element) => {
						this.bind(element, "input", (event) => controls.updateRangeFill.call(player, event.target));
					});
					if (player.config.toggleInvert && !is.element(elements.display.duration)) this.bind(elements.display.currentTime, "click", () => {
		...
		var loadjs = /* @__PURE__ */ getDefaultExportFromCjs(requireLoadjs_umd());
		...
		const vimeo = {
			setup() {
				...
				if (!is.object(window.Vimeo)) loadScript(player.config.urls.vimeo.sdk).then(() => {
			...
			ready() {
				...
				if (!is.empty(referrerPolicy)) iframe.setAttribute("referrerPolicy", referrerPolicy);
				...
				if (!config.customControls) fetch(format(player.config.urls.vimeo.api, src)).then((response) => {
					if (is.empty(response) || !response.thumbnail_url) return;
				...
				player.embed.on("loaded", () => {
					...
					if (is.element(player.embed.element) && player.supported.ui) player.embed.element.setAttribute("tabindex", -1);
				});
		...
		const youtube = {
			setup() {
				...
				if (is.object(window.YT) && is.function(window.YT.Player)) youtube.ready.call(this);
				else {
			...
			ready() {
				...
				if (!is.empty(currentId) && currentId.startsWith("youtube-")) return;
				let source = player.media.getAttribute("src");
				if (is.empty(source)) source = player.media.getAttribute(this.config.attributes.embed.id);
				...
				player.embed = new window.YT.Player(player.media, {
					...
					events: {
						...
						onReady(event) {
							if (is.function(player.media.play)) return;
							const instance = event.target;
		...
		class Ads {
			...
			*/
			constructor(player) {
				...
				*/
				_defineProperty$1(this, "load", () => {
					...
					if (!is.object(window.google) || !is.object(window.google.ima)) loadScript(this.player.config.urls.googleIMA.sdk).then(() => {
						this.ready();
					}).catch(() => {
				...
				*/
				...
				_defineProperty$1(this, "addCuePoints", () => {
					if (!is.empty(this.cuePoints)) this.cuePoints.forEach((cuePoint) => {
				...
				*/
				_defineProperty$1(this, "listeners", () => {
					...
					this.player.on("seeked", () => {
						...
						if (is.empty(this.cuePoints)) return;
				...
				*/
				_defineProperty$1(this, "trigger", (event, ...args) => {
					...
					if (is.array(handlers)) handlers.forEach((handler) => {
						if (is.function(handler)) handler.apply(this, args);
					});
				});
				/**
				* Add event listeners
				...
				*/
				_defineProperty$1(this, "on", (event, callback) => {
					if (!is.array(this.events[event])) this.events[event] = [];
			...
			get tagUrl() {
				const { config } = this;
				if (is.url(config.tagUrl)) return config.tagUrl;
		...
		*/
			...
			constructor(player) {
				...
				_defineProperty$1(this, "getThumbnails", () => {
					return new Promise((resolve) => {
						...
						if (is.empty(src)) throw new Error("Missing previewThumbnails.src config attribute");
						const sortAndResolve = () => {
							this.thumbnails.sort((x, y) => x.height - y.height);
							this.player.debug.log("Preview thumbnails", this.thumbnails);
							resolve();
						};
						if (is.function(src)) src((thumbnails) => {
							this.thumbnails = thumbnails;
							sortAndResolve();
						});
						else {
							const promises = (is.string(src) ? [src] : src).map((u) => this.getThumbnail(u));
				...
				*/
				_defineProperty$1(this, "render", () => {
					...
					if (is.element(this.player.elements.progress)) this.player.elements.progress.appendChild(this.elements.thumb.container);
		...
		const source = {
			insertElements(type, attributes) {
				if (is.string(attributes)) insertElement(type, this.media, { src: attributes });
				else if (is.array(attributes)) attributes.forEach((attribute) => {
			...
			change(input) {
				...
				this.destroy(() => {
					...
					this.media = null;
					if (is.element(this.elements.container)) this.elements.container.removeAttribute("class");
					...
					if (is.boolean(input.autoplay)) this.config.autoplay = input.autoplay;
					if (this.isHTML5) {
		...
		class Plyr {
			constructor(target, options) {
				...
				*/
				_defineProperty$1(this, "play", () => {
					if (!is.function(this.media.play)) return null;
				...
				*/
				_defineProperty$1(this, "pause", () => {
					if (!this.playing || !is.function(this.media.pause)) return null;
				...
				*/
				_defineProperty$1(this, "togglePlay", (input) => {
					if (is.boolean(input) ? input : !this.playing) return this.play();
					return this.pause();
				});
				/**
				* Stop playback
				*/
				_defineProperty$1(this, "stop", () => {
					if (this.isHTML5) {
						this.pause();
						this.restart();
					} else if (is.function(this.media.stop)) this.media.stop();
				});
				/**
				* Restart playback
				*/
				...
				*/
				_defineProperty$1(this, "toggleControls", (toggle) => {
					if (this.supported.ui && !this.isAudio) {
						...
						if (hiding && is.array(this.config.controls) && this.config.controls.includes("settings") && !is.empty(this.config.settings)) controls.toggleMenu.call(this, false);
						if (hiding !== isHidden) {
				...
				_defineProperty$1(this, "destroy", (callback, soft = false) => {
					...
					const done = () => {
						...
						if (soft) {
							...
							if (is.function(callback)) callback.call(this.elements.original);
							this.ready = false;
							setTimeout(() => {
								this.elements = null;
								this.media = null;
					...
					if (this.isHTML5) {
						...
						if (this.embed !== null && is.function(this.embed.destroy)) this.embed.destroy();
						done();
				...
				*/
				...
				this.media = target;
				if (is.string(this.media)) this.media = document.querySelectorAll(this.media);
			...
			*/
			get buffered() {
				...
				if (is.number(buffered)) return buffered;
			...
			*/
			set volume(value) {
				...
				const max = 1;
				const min = 0;
				if (is.string(volume)) volume = Number(volume);
			...
			*/
			set muted(mute) {
				let toggle = mute;
				if (!is.boolean(toggle)) toggle = this.storage.get("muted");
			...
			*/
			set speed(input) {
				let speed = null;
				if (is.number(input)) speed = input;
			...
			*/
			set pip(input) {
				...
				if (is.function(this.media.webkitSetPresentationMode)) this.media.webkitSetPresentationMode(toggle ? pip.active : pip.inactive);
			...
			*/
			static setup(selector, options = {}) {
				let targets = null;
				if (is.string(selector)) targets = Array.from(document.querySelectorAll(selector));
				else if (is.nodeList(selector)) targets = Array.from(selector);
				else if (is.array(selector)) targets = selector.filter(is.element);
...
//#endregion
//#region node_modules/react-plyr/lib/defaultProps.js
var require_defaultProps = /* @__PURE__ */ __commonJSMin(((exports) => {
...
//#endregion
//#region node_modules/react-plyr/lib/index.js
var require_lib = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.default = void 0;
	var _react = _interopRequireWildcard(require_react());
	var _propTypes = _interopRequireDefault(require_prop_types());
	var _plyr = _interopRequireDefault(require_plyr());
	var _defaultProps = _interopRequireDefault(require_defaultProps());
	...
	var Plyr = class extends _react.Component {
		constructor() {
			var _this;
			super(...arguments);
			_this = this;
			...
			_defineProperty(this, "state", { muted: null });
			...
			_defineProperty(this, "renderPlayerWithSRC", () => {
				const _this$props = this.props, sources = _this$props.sources, url = _this$props.url, preload = _this$props.preload, poster = _this$props.poster, className = _this$props.className;
				const captionsMap = _this$props.tracks.map((source, index) => {
					const _source$key = source.key, key = _source$key === void 0 ? index : _source$key, _source$kind = source.kind, kind = _source$kind === void 0 ? "captions" : _source$kind, label = source.label, src = source.src, srclang = source.srclang, def = source.default, attributes = _objectWithoutProperties(source, [
						"key",
						"kind",
						"label",
						"src",
					...
					return _react.default.createElement("track", _extends({
						...
						srclang,
						default: def
					}, attributes, { ref: this.elementRef }));
				});
				if (sources && Array.isArray(sources) && sources.length) return _react.default.createElement("video", {
					className,
					preload,
					poster,
					ref: this.elementRef
			...
			_defineProperty(this, "renderAudioPlayer", () => {
				const _this$props2 = this.props, sources = _this$props2.sources, url = _this$props2.url, preload = _this$props2.preload, className = _this$props2.className;
				if (sources && Array.isArray(sources) && sources.length) return _react.default.createElement("audio", {
					className,
					preload,
					ref: this.elementRef
				}, sources.map((source, index) => _react.default.createElement("source", {
		...
		componentDidMount() {
			const options = _objectSpread({}, Object.keys(_defaultProps.default).reduce((acc, current) => _objectSpread({}, acc, { [current]: this.props[current] }), {}), { muted: this.state.muted });
			const node = this.elementRef.current;
			this.player = node ? new _plyr.default(node, options) : null;
			if (this.player) {
				this.player.on("ready", () => {
					this.props.onReady && this.props.onReady(this.player);
					if (this.props.autoplay) this.player.play();
				});
				this.player.on("play", () => {
				...
				this.player.on("volumechange", () => {
					const _this$player = this.player, muted = _this$player.muted, volume = _this$player.volume;
		...
		componentDidUpdate(prevProps) {
			if (prevProps.muted !== this.props.muted) this.player.muted = this.props.muted;
			if (prevProps.videoId !== this.props.videoId) this.props.videoId && this.updateVideoSource(this.props.videoId, this.props.provider);
		}
		componentWillUnmount() {
			this.player && this.player.destroy();
		}
		render() {
			if (this.props.type === "audio") return this.renderAudioPlayer();
			else if (this.props.type === "video") return this.renderPlayerWithSRC();
			return this.renderPlayerWithVideoId();
		}
	};
	_defineProperty(Plyr, "getDerivedStateFromProps", (nextProps) => ({ muted: nextProps.muted }));
	...
	_defineProperty(Plyr, "propTypes", {
		...
		loop: _propTypes.default.shape({ active: _propTypes.default.bool }),
		...
		settings: _propTypes.default.arrayOf(_propTypes.default.oneOf([
			"captions",
			"quality",
			"speed"
		])),
...
//# sourceMappingURL=react-plyr.js.map